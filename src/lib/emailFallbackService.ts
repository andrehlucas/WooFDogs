...
function formatLeadEmailContent(lead: InsertLead): string {
...
<body>
  <div class="alert-box">
    <h1>ATTENTION: Perfex Integration Failed</h1>
  ...
  <div class="instructions">
    ...
    <ol>
      <li>Log in to Perfex CRM at <a href="https://crm.woofdogs.com">https://crm.woofdogs.com</a></li>
...
export async function sendFallbackEmail(lead: InsertLead): Promise<boolean> {
  try {
    console.log('[FALLBACK] Getting Gmail client...');
    const gmail = await getUncachableGmailClient();
    
    const safeName = escapeHtml(lead.name || 'Unknown');
    const subject = `URGENT: NEW LEAD (Perfex Failed) - ${safeName} - Action Required`;
    const htmlContent = formatLeadEmailContent(lead);
    
    console.log('[FALLBACK] Sending email to:', FALLBACK_RECIPIENTS.join(', '));
    ...
    console.error('[FALLBACK] Lead data for manual recovery:', JSON.stringify(lead, null, 2));
    return false;
  }
}
