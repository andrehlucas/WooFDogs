...
function NavigationMenuLink({
        ...
        return (
                <NavigationMenuPrimitive.Link
                        ...
                        className={cn(
                                "data-[active=true]:focus:bg-accent data-[active=true]:hover:bg-accent data-[active=true]:bg-accent/50 data-[active=true]:text-white hover:bg-accent hover:text-white hover:[&_svg]:text-white focus:bg-accent focus:text-white focus:[&_svg]:text-white focus-visible:ring-ring/50 [&_svg:not([class*='text-'])]:text-muted-foreground flex flex-col justify-center gap-1 rounded-sm px-4 py-1 text-sm transition-all outline-none focus-visible:ring-[3px] focus-visible:outline-1 [&_svg:not([class*='size-'])]:size-4",
                                className,
                        )}
                        {...props}
                />
...
function NavGridCard({
        ...
        return (
                <a href={link.href} className="block no-underline">
                        <GridCard {...props}>
                                {link.icon && (
                ...
                >
                        <div className="flex items-center justify-between px-5 py-4">
                                <div className="space-y-1">
                                        <span className="text-sm leading-none font-medium group-hover:text-white transition-colors">{link.title}</span>
                                        {link.description && (
                                                <p className="text-muted-foreground group-hover:text-white/90 line-clamp-1 text-xs transition-colors">
                                                        {link.description}
                                                </p>
                                        )}
                                </div>
                                {link.icon && <link.icon className="text-muted-foreground group-hover:text-white size-8 transition-colors" />}
                        </div>
                </NavigationMenuLink>
        );
}
...
function NavItemMobile({
        ...
        return (
                <a
                        className={cn(
                                "data-[active=true]:focus:bg-accent data-[active=true]:hover:bg-accent data-[active=true]:bg-accent/50 data-[active=true]:text-white hover:bg-accent hover:text-white hover:[&_svg]:text-white hover:[&_.text-muted-foreground]:text-white/80 focus:bg-accent focus:text-white focus:[&_svg]:text-white focus:[&_.text-muted-foreground]:text-white/80 focus-visible:ring-ring/50 [&_svg:not([class*='text-'])]:text-muted-foreground group relative flex gap-1 gap-x-2 rounded-sm p-2 text-sm transition-all outline-none focus-visible:ring-[3px] focus-visible:outline-1 [&_svg:not([class*='size-'])]:size-4",
                                className,
                        )}
                        {...props}
                >
