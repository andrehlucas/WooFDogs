...
function CardInner({ card, index }: { card: CardItem; index: number }) {
  return (
    <div className="relative w-full h-full overflow-hidden" style={{ borderRadius: 22 }}>
      ...
      {/* Smooth gradient overlay — pure CSS gradient, no backdrop-filter so it's
          identical on every card regardless of z-index or GSAP transforms */}
      {(card.title || card.ctaText) && (
        <div
          ...
          style={{
            ...
            background: [
              ...
              "  rgba(238,228,232,0)    0%,",
              "  rgba(238,228,232,0.06) 12%,",
              "  rgba(237,226,231,0.18) 24%,",
              "  rgba(236,224,229,0.38) 37%,",
              "  rgba(234,221,227,0.62) 51%,",
              "  rgba(232,218,224,0.80) 64%,",
              "  rgba(230,216,222,0.90) 76%,",
              "  rgba(228,214,220,0.94) 88%,",
              "  rgba(226,212,218,0.96) 100%",
              ")",
            ].join(" "),
          }}
        />
      ...
      {(card.title || card.ctaText) && (
        <div
          className="absolute inset-x-0 bottom-0 flex flex-col items-center gap-[6px]"
          style={{ padding: "0 12px 14px" }}
        >
          {card.title && (
            <p
              className="font-bold text-center leading-snug"
              style={{ fontSize: "0.65rem", color: "#222222" }}
            >
              {card.title}
            </p>
          )}
